...
PMOCore is a Project Management & Operations platform. This repository contains the AUTH-01 foundation plus **Solo MVP Package 1**: the complete Solo MVP database schema and reference data, PostgreSQL-backed session authentication, project management (create, list, view, update, archive), the authenticated responsive application shell, shared contracts for every module, and production single-service deployment support.

Meetings & Visits, Actions/RAID, attachments, plan, requirements, testing, releases, documents and the dashboard are delivered by later packages (see `.kiro/specs/SOLO-MVP-01/tasks.md`).
...
| `TEST_DATABASE_URL` | Local integration tests only: a **separate** database whose name ends in `_test`. Tests drop and rebuild its schema; the reset helper refuses any other database name. Never point it at a shared or production database. |
| `APP_ORIGIN` | Comma-separated browser origin(s) allowed to make state-changing requests (CSRF guard). Defaults to `http://localhost:5173`; **required in production** (e.g. `https://pmocore.onrender.com`). |
| `APP_TIMEZONE` | IANA time zone for business "today" (overdue, next milestone). Default `Asia/Manila`. |
| `SESSION_SECRET` | **Required.** Random secret, at least 32 characters, used to sign the session cookie. |
| `SESSION_IDLE_DAYS` / `SESSION_ABSOLUTE_DAYS` | Session idle timeout (rolling) and absolute lifetime. Defaults `7` / `30`. |
| `ATTACHMENT_STORAGE_DRIVER` | `local` (development/tests) or `s3` (staging/production, Cloudflare R2). Default `local`. |
| `ATTACHMENT_STORAGE_DIR` | Root directory for the `local` driver. Default `./.data/attachments` (git-ignored). |
| `ATTACHMENT_MAX_BYTES` / `ATTACHMENT_PURGE_DAYS` | Upload limit (default and maximum 10 MB) and soft-delete retention before purge (default 30 days). |
| `S3_ENDPOINT`, `S3_REGION`, `S3_BUCKET`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY`, `S3_FORCE_PATH_STYLE` | S3-compatible storage settings; required when `ATTACHMENT_STORAGE_DRIVER=s3`. |

The API variables (all of the above except `DATABASE_URL`/`DATABASE_SSL`/`TEST_DATABASE_URL`) are validated at API startup with a Zod schema in `apps/api/src/config/env.ts`. `DATABASE_URL` and `DATABASE_SSL` are validated separately, with a Zod schema owned by `@pmocore/database` (`database/src/config.ts`), since that package creates the PostgreSQL connection and may be consumed independently of the API. Either module exits immediately with a clear error on missing or invalid required configuration — the application never starts in a partially-configured state — and neither ever logs or exposes the configured values themselves.
...
- **Testing behavior.** Vitest runs in `apps/api`, `apps/web`, `packages/shared`, and `database`. Integration tests (schema, migrations, seed, authentication, sessions, throttling, CSRF, project access and CRUD, route protection) use a separate local PostgreSQL database from `TEST_DATABASE_URL`; the API suite rebuilds it once per run and truncates data between tests. Create it once as a PostgreSQL superuser, e.g. `CREATE DATABASE pmocore_test OWNER <app-role>;`. Only synthetic credentials are used. `apps/web` covers the API client, route guard, login, listings and project forms with React Testing Library.
...
The approved V1 deployment is a single Render Web Service (API + built frontend) with Neon PostgreSQL. The exact procedure, environment settings and verification checklist are in [`docs/architecture/deployment.md`](docs/architecture/deployment.md).
