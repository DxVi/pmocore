...
- **Testing behavior.** Vitest runs in `apps/api`, `apps/web`, and `packages/shared`. `apps/api` currently has the one required test (`GET /api/health`, covering both a reachable and an unreachable database, via Supertest against the exported Express app — no real network or database connection is used). `apps/web` and `packages/shared` have no test files yet; their Vitest configs use `passWithNoTests` so an empty suite is a clean pass rather than a failure. `database` has no automated tests in this phase.
